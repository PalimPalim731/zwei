class User < ActiveRecord::Base
  ...
  def self.signup(params)
    puts "\n\n ^^^^ We will signup the user with: Name: #{params[:name]};
  ...
  def self.authenticate(params)
    puts "\n\n ^^^^ We will authenticate the user with the email: #{params[:email]}
    ...
    if params[:email] and !params[:email].blank?
      ...
      # is the same with the password submitted after we encrypt it
      
      if user and user.password == params[:password]
        puts "\n\n ^^^^ We've found the user with the email #{params[:email]} 
        and password #{params[:password]}. Log them in. \n\n"
        return user
      end
    end
    
    puts "\n\n ^^^^ We have not found the requested user, we'll just return a blank user \n\n"
    # just return an empty user
    self.new
  end
